import { getImageUri, log } from "@/actions/helpers";
import * as API from "@/lib/scryfallApi";
import { Prisma } from "@prisma/client";
import { LogLevel } from "next-axiom/dist/logger";
import { Card, Set } from "scryfall-sdk";
...
export const createCardsDetails = async (cardsToAdd: Card[], set: Set) => {
  // Create CardDetails
  log(LogLevel.info, "Creating card details for album " + set.name);
  await prisma.cardDetails.createMany({
    skipDuplicates: true,
  ...
  // Create CardFaces
  log(LogLevel.info, "Creating card faces for album " + set.name);
  const cardFacesToCreate: Prisma.CardFaceCreateManyInput[] = [];
  cardsToAdd.forEach((card) => {
    const faces = card.card_faces;
    if (faces.length > 1) {
      faces.forEach((face, idx) => {
        cardFacesToCreate.push({
          cardId: card.id,
          faceNumber: idx,
          artist: face.artist,
  ...
  await prisma.cardFace.createMany({
    skipDuplicates: true,
    data: cardFacesToCreate,
  });
};
